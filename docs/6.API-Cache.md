## REST API에 캐쉬를 적용시켜 보자.

1. 설치
2. CASE1 글로벌 캐쉬 적용
3. CASE2 개별 API 캐쉬 적용
...
- 수동으로 캐쉬 key를 관리하여, 특정 요청에대해서는 캐슁 처리를 하자.
- url에 대해서 캐슁하는 예
...
// (1) 캐쉬정책 : originalUrl (Params/query) 이 동일하면 캐슁
@Injectable()
...
// (2) 캐쉬정책 : protocol://hostname+originalUrl 이 동일하면 캐슁
@Injectable()
...
// (3) 캐쉬정책 : originalUrl (Params/query) + Body 동일하면 캐슁
@Injectable()
...
- "/api/finance/corporations/00{}" 라고 캐쉬 키가 잡히는 모습
- "/api/finance/dailystock/006400?take=10{}" 라고 캐쉬 키가 잡히는 모습

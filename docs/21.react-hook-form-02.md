...
const TEST4 = () => {
  ...
  const { append, fields, insert, move, prepend, remove, swap, update } =
    useFieldArray({
      ...
      name: `todo`, // as const, // unique name for your Field Array
          ...
          return (
            <li key={item.id}>
              <input
                ...
                {...register(`todo.${index}.content` as const)}
              />
              <input
                type="checkbox"
                defaultChecked={item.finished} // make sure to set up defaultValue
                {...register(`todo.${index}.finished` as const)}
              />
